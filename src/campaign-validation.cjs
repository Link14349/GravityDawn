...
for(let index=0;index<levels.length;index++){
  ...
  for(let f=0;f<1800;f++){
    ...
    if(f%60===0)for(const bullet of idle.bullets){
  ...
  results.push({id:level.id,name:level.name,fingerprint:crypto.createHash('sha256').update(JSON.stringify(level)).digest('hex'),
    ...
    idleSeconds:30,passed:result.passed,stars:result.stars,shots:sim.getPerformance().shotsUsed,fullThrustReplacementPassed:fullPassed,maxThrustSweeps:sweeps});
...
console.log('PASS 36 mission replays, stable parking orbits, maximum-thrust counterchecks, moving frames, exact contacts, payloads, input and save regressions.');
