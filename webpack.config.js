...
const demoPages = ['test/phase4-demo.html', 'test/phase9-demo.html', 'test/phase10-demo.html', 'test/phase11-demo.html', 'test/phase12-demo.html', 'test/phase13-demo.html', 'test/phase14-demo.html', 'test/phase15-demo.html', 'test/phase16-demo.html', 'test/phase17-demo.html', 'test/phase18-demo.html', 'test/phase19-demo.html', 'test/phase20-demo.html', 'test/phase21-demo.html', 'test/phase22-demo.html', 'test/phase23-demo.html'];
const pageKey = file => file === 'index.html' ? 'app' : path.basename(file, '.html');

module.exports = (_env, argv = {}) => {
  const production = argv.mode === 'production';
  const pages = ['index.html', ...(production ? [] : demoPages)].filter(file => fs.existsSync(path.resolve(__dirname, file)));
  return {
    mode: production ? 'production' : 'development',
    entry: Object.fromEntries(pages.map(file => [pageKey(file), `./src/page-entry-loader.cjs!./${file}`])),
    output: {
      filename: production ? '[name].[contenthash:8].js' : '[name].bundle.js',
    ...
    plugins: [
      ...pages.map(file => new HtmlWebpackPlugin({
        ...
        chunks: [pageKey(file)],
        inject: 'body',
        meta: { 'application-version': version },
      })),
