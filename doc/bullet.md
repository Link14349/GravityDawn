...
弹体实心圆和外圈统一使用 `Bullet.color`：优先使用关卡显式 `color`，否则使用类型字典颜色。待发射、飞行及悬停时均保留该颜色；弹体实心圆始终带细浅色描边，确保深色弹体在深空背景上可见。爆炸弹使用明亮的珊瑚粉，提升背景对比度。待发射白色描边与悬停光环继续表示交互状态，不覆盖类型颜色。动能弹默认渲染半径 16，是普通弹 8 的两倍，便于区分两种红色弹药。分裂子弹继承母弹颜色（默认青色）。`test/phase13-demo.html` 调用实际 `Bullet` 与 `Renderer` 展示七种弹药及各状态。
