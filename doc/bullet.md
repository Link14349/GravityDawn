...
| `explosive` | 爆炸弹 | 300 | 2 | 200 | 4000 | 14 | `#4488ff` 蓝 | 0 | — |
| `kinetic` | 动能弹 | 220 | 2 | 0 | 0 | 16 | `#44ddee` 青 | 300 | 无爆炸，靠撞击+HP生存 |
| `agile` | 机动弹 | 1000 | 10 | 30 | 2000 | 8 | `#ffdd44` 黄 | 0 | 高ΔV+多次点火 |
| `cluster` | 分裂弹 | 280 | 2 | 25 | 1000 | 8 | `#44ff88` 绿 | 0 | 分裂 3 颗小子弹(不可机动) |
...
弹体实心圆和外圈统一使用 `Bullet.color`：优先使用关卡显式 `color`，否则使用类型字典颜色。待发射、飞行及悬停时均保留该颜色；待发射白色描边与悬停光环继续表示交互状态，不覆盖类型颜色。分裂子弹保留自身配置的浅绿色。`test/phase13-demo.html` 调用实际 `Bullet` 与 `Renderer` 展示七种弹药及各状态。
...
- 松开鼠标 → 发射（消耗一次点火和相应 ΔV）
- 可多次点火（`remainingIgnitions > 0` 且 `remainingDeltaV > 0`）
- 悬停半径 40px，ΔV 缩放 0.5（拖动像素→ΔV 单位）
...
- 数量：3 颗
- 类型：normal 变体（renderRadius=5, 绿色 `#88ffaa`）
