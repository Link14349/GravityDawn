...
- Briefings: two-column scientific diagram and transmission, typed dialogue, progress segments, explicit continue / skip. The header uses the same 44 × 44px planet-and-dawn logo as the archive / result bar.
...
瞄准时以橙色圆点线圈标记剩余 ΔV 上限，圈心位于弹体，圈内标注「拖出点线圈取消」。悬停可机动弹体和拖拽时显示，圆周半径与实际拖拽判定共享世界坐标，随镜头缩放。超出圆周立即清除规划预览；飞行手册同步说明此取消方式。
