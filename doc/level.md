...
`flightTimeout: 30` 与 1800 步预测相配合，允许低推力进近及绕行。弹体飞出 ±1700 世界边界也可进入结算缓冲；目标全部清除仍触发原有的 5 秒结果分析。飞行超时与结果分析均使用模拟时间，随 1×–10× 时间倍率加速，暂停时不推进；固定物理步长与预测窗口保持不变。拖拽时显示点火后燃料与预计内舱清除范围（剩余爆炸半径的一半），不预测未来全部碎片碰撞。
