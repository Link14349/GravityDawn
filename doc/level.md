...
`flightTimeout: 30` 与 1800 步预测相配合，允许低推力进近及绕行。弹体飞出 ±1700 世界边界也可进入结算缓冲；目标全部清除仍触发原有的 5 秒结果分析。飞行超时与结果分析均使用模拟时间，随时间滑动条的 0.5×、1×、1.5×、2×、4×、5×、10×、20× 档位变速，暂停时不推进；固定物理步长与预测窗口保持不变。拖拽时显示点火后燃料与预计内舱清除范围（剩余爆炸半径的一半），不预测未来全部碎片碰撞。

移动行星通过星体 `orbit` 的 `circular` / `elliptical` 参数定义，卫星的 `parent` 可继续指向另一颗移动行星的轨道。随行哨站使用建筑级 `orbit`，并把其 `parent` 指向宿主行星轨道；这只是平移参考系，构件仍参与弹簧模拟。
